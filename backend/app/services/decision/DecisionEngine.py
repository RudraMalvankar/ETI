...
class DecisionEngine:
    ...
    def make_decision(self, request: DecisionRequest) -> DecisionResponse:
        ...
        retrieved_chunks = self.retriever.retrieve_context(
            request.failed_asset, request.failure_type
        )

        # 2 & 3 & 4. Collect graph results, simulation outputs and Assemble context
        context = self.context_builder.build_context(
            request.failed_asset, request.simulation_id, retrieved_chunks
        )
        ...
        valid_citations = self.citation_resolver.resolve_citations(
            parsed_json.get("supporting_citations", []), retrieved_chunks
        )
