...
class ComplianceValidator:
    ...
    def validate(self, memory: IncidentMemory) -> List[str]:
        ...
        has_isolation = any(
            "isolate" in str(step.get("title", "")).lower()
            or "safety" in str(step.get("title", "")).lower()
            for step in memory.runbook_history
        )
        if has_isolation:
            checklist.append(
                "[PASS] Safety Protocols: Electrical and pressure isolation steps verified in runbook."
            )
        else:
            checklist.append(
        ...
        checklist.append(
            f"[PASS] Audit Closure: Incident finalized with outcome '{memory.outcome}'."
        )
