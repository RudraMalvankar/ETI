...
def print_scenario_summary(scenario):
    ...
    print(f"   Affected Assets: {len(scenario['affected_assets'])} -> {scenario['affected_assets']}")
    print(f"   Safety Level: {scenario['safety_level']}")
    print(f"   Downtime: {scenario['estimated_downtime_hours']} hrs | Cost: ${scenario['estimated_cost_usd']}")
    risk = scenario['risk_score']
    print(f"   Risk -> Safety: {risk['safety_risk']}, Ops: {risk['operational_risk']}, Fin: {risk['financial_risk']}, Env: {risk['environmental_risk']} | OVERALL: {risk['overall_score']}")
