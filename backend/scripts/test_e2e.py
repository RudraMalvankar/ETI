...
def run_e2e():
    ...
    res, lat = measure_latency(requests.post, f"{BASE_URL}/documents/index", json={"document_id": doc_id})
    print(f"  Index Latency: {lat:.2f}ms | Status: {res.status_code}")
    print(f"  Index Response: {res.json()}")
    
    # 7. Semantic Retrieval
    print("\n[7] Semantic Retrieval...")
    res, lat = measure_latency(requests.post, f"{BASE_URL}/search/", json={"query": "P-101 failure", "top_k": 2})
    ...
    for sc in scenarios:
        print(f"    - {sc['name']}: {len(sc['affected_assets'])} assets affected, Score: {sc['risk_score']['overall_score']}")
        
    # 12-15. Decision Engine
    print("\n[12-15] Decision Engine...")
    req_dec = {"failed_asset": "P-101", "failure_type": "mechanical_failure", "simulation_id": sim_id}
