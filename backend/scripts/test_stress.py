...
def run_stress_tests():
    ...
    res = requests.post(f"{BASE_URL}/documents/upload", files={"file": ("invalid.txt", io.BytesIO(b"Not a PDF"), "text/plain")})
    print_test("Invalid File Type (txt)", res)

    res = requests.post(f"{BASE_URL}/documents/upload", files={"file": ("empty.pdf", io.BytesIO(b""), "application/pdf")})
    print_test("Empty File", res)

    res = requests.post(f"{BASE_URL}/documents/upload", files={"file": ("corrupt.pdf", io.BytesIO(b"%PDF-1.4\ncorrupted content"), "application/pdf")})
    ...
    res = requests.post(f"{BASE_URL}/simulation/run", json={"failed_asset": "UNKNOWN-PUMP", "failure_type": "explode"})
    ...
    print("\n--- Decision Engine Edge Cases ---")
    res = requests.post(f"{BASE_URL}/decision/recommend", json={"failed_asset": "P-101", "failure_type": "break", "simulation_id": "invalid-sim-id"})
