...
def run_test():
    ...
    mock_decision_payload = {
        ...
        "supporting_citations": [
            {"document_id": "doc123", "chunk_id": "chunk456", "text_snippet": "P-101 failure protocol requires isolation."}
        ],
    ...
    res = requests.put(f"{BASE_URL}/runbook/{rb_id}/step/{step_1_id}", json={"status": "completed", "feedback_notes": "Valves closed."})
    rb = res.json()
    ...
    res = requests.put(f"{BASE_URL}/runbook/{rb_id}/step/{step_2_id}", json={"status": "failed", "feedback_notes": "Valve is rusted shut, cannot isolate."})
    rb = res.json()
