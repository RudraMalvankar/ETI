...
export const CausalGraphView: React.FC = () => {
  const { nodes, edges, selectedNode, setSelectedNode, isAnomalyActive, isRerouted } = useApexStore();
  ...
  const getNodeIcon = (type: PlantNode['type']) => {
    switch (type) {
      case 'reactor': return <Activity className="w-4 h-4" />;
      case 'sensor': return <Cpu className="w-4 h-4" />;
      case 'valve': return <ShieldAlert className="w-4 h-4" />;
      case 'pump': return <Zap className="w-4 h-4" />;
      case 'equipment': return <Flame className="w-4 h-4" />;
      ...
      <div className="absolute top-4 left-4 right-4 z-10 flex flex-wrap items-center justify-between gap-3 pointer-events-none">
        <div className="bg-[#111827]/90 backdrop-blur-md px-3.5 py-2 rounded-lg border border-slate-800 pointer-events-auto flex items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-white font-mono">Causal Propagation Graph</span>
          </div>
          <div className="h-3 w-[1px] bg-slate-700"></div>
          <span className="text-[11px] text-slate-400">
            {isRerouted ? 'Blast Radius: Rerouted via Bypass V-108' : isAnomalyActive ? 'Blast Radius: 3 Assets Impacted' : 'Blast Radius: 0 (Nominal)'}
          </span>
        </div>
            ...
            return (
              ...
              >
                <foreignObject width="180" height="75">
                  <div className={`p-2.5 rounded-xl border transition-all ${style.bg} ${style.border} ${style.glow} shadow-lg ${
                    isSelected ? 'ring-2 ring-blue-400 border-blue-400 scale-105' : 'hover:scale-102 hover:border-slate-400'
                  }`}>
                    <div className="flex items-center justify-between gap-1 mb-1">
                      <div className="flex items-center gap-1.5">
                        <span className={`w-2 h-2 rounded-full ${style.dot} ${node.status === 'critical' ? 'animate-ping' : ''}`}></span>
                        <span className={`text-[11px] font-bold font-mono ${style.text}`}>{node.designSpecs?.tag || node.id}</span>
                      </div>
                    ...
                    {node.telemetry?.pressure && (
                      <div className="text-[10px] font-mono text-slate-300 mt-1 flex items-center justify-between bg-black/40 px-1.5 py-0.5 rounded">
                        <span>P:</span>
                        <span className={node.status === 'critical' ? 'text-red-400 font-bold' : 'text-slate-300'}>{node.telemetry.pressure}</span>
                      </div>
                    )}
                  </div>
                </foreignObject>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className={`px-2 py-0.5 rounded text-[10px] font-mono uppercase font-bold ${getNodeColor(selectedNode.status).text} bg-slate-900 border border-slate-800`}>
                  {selectedNode.status}
                </span>
              </div>
              <p className="text-slate-400 text-[11px]">Tag: {selectedNode.designSpecs?.tag} | Manual: {selectedNode.designSpecs?.oemManual}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 text-[11px] font-mono">
            ...
            {selectedNode.telemetry?.temperature && (
              <div className="bg-slate-900 px-3 py-1.5 rounded-lg border border-slate-800">
                ...
                <span className="text-emerald-400 font-bold">{selectedNode.telemetry.temperature}</span>
              </div>
            )}
