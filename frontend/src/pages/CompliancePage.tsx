      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-3">
            ...
            >
              {isDownloadingPdf ? <span className="w-3.5 h-3.5 border-2 border-[var(--text-primary)] border-t-transparent rounded-full animate-spin" /> : <Download className="w-3.5 h-3.5 text-accent-red" />}
              PDF Report
            </button>
            <button 
              ...
              className="flex items-center gap-2 px-4 py-2 bg-[var(--bg-secondary)] hover:bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-xl text-xs font-bold text-[var(--text-primary)] transition-all"
            >
              {isDownloadingDocx ? <span className="w-3.5 h-3.5 border-2 border-[var(--text-primary)] border-t-transparent rounded-full animate-spin" /> : <FileText className="w-3.5 h-3.5 text-brand-400" />}
              DOCX Report
            </button>
          </div>
        }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="p-6 rounded-3xl bg-[var(--bg-secondary)] border border-[var(--glass-border)] shadow-lg flex items-center gap-6">
          <div className="w-20 h-20 rounded-full border-[6px] border-accent-emerald flex items-center justify-center relative shadow-[0_0_20px_rgba(16,185,129,0.3)]">
            <span className="text-2xl font-extrabold text-white">98<span className="text-sm">%</span></span>
          </div>
          <div>
            <h3 className="text-sm font-bold text-[var(--text-primary)]">Enterprise Compliance Score</h3>
            <p className="text-xs text-[var(--text-secondary)] mt-1 flex items-center gap-1"><CheckCircle2 className="w-3.5 h-3.5 text-accent-emerald" /> Audit Ready Status</p>
          </div>
        </div>

        <div className="p-6 rounded-3xl bg-[var(--bg-secondary)] border border-[var(--glass-border)] shadow-lg flex flex-col justify-center">
          <h3 className="text-sm font-bold text-[var(--text-primary)] flex items-center gap-2"><FileText className="w-4 h-4 text-brand-400" /> Regulatory Mapping</h3>
        ...
        <div className="p-6 rounded-3xl bg-accent-amber/5 border border-accent-amber/20 shadow-lg flex flex-col justify-center text-accent-amber">
          <h3 className="text-sm font-bold flex items-center gap-2"><AlertTriangle className="w-4 h-4" /> Missing Evidence / Alerts</h3>
          <p className="text-xs mt-2 leading-relaxed">
            1 pending manual sign-off required for incident <strong className="bg-accent-amber/20 px-1 rounded">REP-FB573B93</strong>. Please ensure the lead technician completes the LOTO visual inspection upload.
          </p>
        </div>
      </div>
      ...
      <div className="p-8 rounded-3xl bg-[var(--bg-secondary)] border border-[var(--glass-border)] shadow-2xl space-y-8 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between pb-6 border-b border-[var(--glass-border)] gap-4 relative z-10">
          <div>
            ...
            <span className="text-xs text-[var(--text-secondary)] font-mono mt-2 block bg-[var(--bg-primary)] inline-block px-3 py-1 rounded-md border border-[var(--glass-border)]">Serial ID: {report.report_id}</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
          ...
          <div className="p-5 rounded-2xl bg-[var(--bg-primary)] border border-[var(--glass-border)]">
            <h4 className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-wider mb-3">1. Root Cause Identification</h4>
            <p className="text-white leading-relaxed font-mono text-xs">{report.root_cause}</p>
          </div>

          {/* Final Resolution */}
          <div className="p-5 rounded-2xl bg-[var(--bg-primary)] border border-[var(--glass-border)]">
            <h4 className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-wider mb-3">2. Final Audit Resolution</h4>
            <p className="text-accent-emerald font-semibold leading-relaxed font-mono text-xs">{report.final_resolution}</p>
          </div>
        </div>
        ...
        <div className="space-y-4 relative z-10">
          <h4 className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-wider">4. Deterministic Supporting Evidence</h4>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {report.supporting_evidence.map((ev, idx) => (
              <div key={idx} className="p-5 rounded-2xl bg-[var(--bg-primary)] border border-[var(--glass-border)] text-xs transition-colors hover:border-brand-500/30 group">
                ...
                <span className="inline-block px-2 py-0.5 rounded-md bg-[var(--bg-secondary)] border border-[var(--glass-border)] text-[9px] text-[var(--text-secondary)] font-mono mb-3 uppercase">{ev.category}</span>
        ...
        <div className="p-6 rounded-3xl bg-[var(--bg-primary)] border border-[var(--glass-border)] relative z-10">
          <h4 className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-wider mb-4">5. Regulatory Compliance Checklist</h4>
          <div className="space-y-3">
            {report.compliance_checklist.map((check, idx) => (
              <div key={idx} className="flex items-start gap-3 p-3.5 rounded-xl bg-[var(--bg-secondary)] border border-[var(--glass-border)] text-xs text-white font-mono hover:border-accent-emerald/30 transition-colors">
