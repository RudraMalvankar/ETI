...
export const SettingsPage: React.FC = () => {
    const { apiBaseUrl, connectionState, isDarkMode, toggleTheme, confidenceThreshold, setConfidenceThreshold } = useApexStore();
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {/* Engine Parameters */}
          <SectionCard title="AI Guardrail & Confidence Parameters" subtitle="Decision Engine Hallucination Thresholds">
            <div className="space-y-4 text-xs">
              <div>
                <label className="block text-slate-400 font-semibold mb-1">Minimum Citation Confidence Threshold ({confidenceThreshold}%)</label>
                <input
                  type="range"
                  min="50"
                  max="99"
                ...
                />
                <span className="text-[10px] text-slate-500 mt-1 block">Requires minimum {confidenceThreshold}% citation similarity score</span>
              </div>
        ...
        <SectionCard title="Interface & Theme Mode" subtitle="Customize dashboard appearance">
          <div className="flex items-center justify-between p-4 rounded-xl bg-slate-950/60 border border-slate-800 text-xs">
            <div>
              ...
              <span className="text-slate-400 text-[11px]">{isDarkMode ? 'Enterprise Dark Mode (Recommended)' : 'Light Mode'}</span>
            </div>
            <button
              onClick={toggleTheme}
