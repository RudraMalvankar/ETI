...
export const MOCK_GRAPH_INITIAL: { nodes: PlantNode[]; edges: PlantEdge[] } = {
  nodes: [
    { id: 'R-101', label: 'Primary Reactor R-101', type: 'reactor', status: 'critical', telemetry: { pressure: '18.4 Bar (MAX: 15.0)', temperature: '285°C' }, designSpecs: { maxPressure: '15.0 Bar', oemManual: 'OEM-R101-REV3.pdf', tag: 'R-101' }, x: 120, y: 180 },
    { id: 'PT-401', label: 'Pressure Sensor PT-401', type: 'sensor', status: 'critical', telemetry: { pressure: '18.4 Bar' }, designSpecs: { oemManual: 'SENS-PT401.pdf', tag: 'PT-401' }, x: 300, y: 120 },
    { id: 'V-102', label: 'Emergency Isolation Valve V-102', type: 'valve', status: 'failed', telemetry: { flow: '0 L/min (STUCK OPEN)' }, designSpecs: { oemManual: 'VALVE-V102-SPEC.pdf', tag: 'V-102' }, x: 480, y: 180 },
    { id: 'V-108', label: 'Bypass Relief Valve V-108', type: 'valve', status: 'warning', telemetry: { flow: 'Standby' }, designSpecs: { oemManual: 'VALVE-V108-BYPASS.pdf', tag: 'V-108' }, x: 480, y: 300 },
    { id: 'P-202A', label: 'Cooling Water Pump P-202A', type: 'pump', status: 'nominal', telemetry: { flow: '420 L/min' }, designSpecs: { oemManual: 'PUMP-P202A-OEM.pdf', tag: 'P-202A' }, x: 660, y: 180 },
    { id: 'FS-01', label: 'Flare Stack Diverter FS-01', type: 'equipment', status: 'nominal', telemetry: { flow: '0 L/min' }, designSpecs: { oemManual: 'FLARE-FS01-SPEC.pdf', tag: 'FS-01' }, x: 660, y: 300 }
  ],
  edges: [
...
export const MOCK_GRAPH_REROUTED: { nodes: PlantNode[]; edges: PlantEdge[] } = {
  nodes: [
    { id: 'R-101', label: 'Primary Reactor R-101', type: 'reactor', status: 'warning', telemetry: { pressure: '15.2 Bar (DROPPING)', temperature: '270°C' }, designSpecs: { maxPressure: '15.0 Bar', oemManual: 'OEM-R101-REV3.pdf', tag: 'R-101' }, x: 120, y: 180 },
    { id: 'PT-401', label: 'Pressure Sensor PT-401', type: 'sensor', status: 'warning', telemetry: { pressure: '15.2 Bar' }, designSpecs: { oemManual: 'SENS-PT401.pdf', tag: 'PT-401' }, x: 300, y: 120 },
    { id: 'V-102', label: 'Emergency Isolation Valve V-102', type: 'valve', status: 'failed', telemetry: { flow: 'ISOLATED/BLOCKED' }, designSpecs: { oemManual: 'VALVE-V102-SPEC.pdf', tag: 'V-102' }, x: 480, y: 180 },
    { id: 'V-108', label: 'Bypass Relief Valve V-108', type: 'valve', status: 'critical', telemetry: { flow: '650 L/min (ACTIVE RELIEF)' }, designSpecs: { oemManual: 'VALVE-V108-BYPASS.pdf', tag: 'V-108' }, x: 480, y: 300 },
    { id: 'P-202A', label: 'Cooling Water Pump P-202A', type: 'pump', status: 'nominal', telemetry: { flow: '550 L/min (FULL COOLING)' }, designSpecs: { oemManual: 'PUMP-P202A-OEM.pdf', tag: 'P-202A' }, x: 660, y: 180 },
    { id: 'FS-01', label: 'Flare Stack Diverter FS-01', type: 'equipment', status: 'warning', telemetry: { flow: '650 L/min (VENTING)' }, designSpecs: { oemManual: 'FLARE-FS01-SPEC.pdf', tag: 'FS-01' }, x: 660, y: 300 }
  ],
  edges: [
  ...
  steps: [
    {
      ...
      description: 'Check local position indicator on Valve V-102. Attempt manual override wheel turning counter-clockwise.',
    ...
    },
    {
      ...
      description: 'Apply LOTO Lock ID #LOTO-901 to feed valve handle and verify zero mechanical backlash.',
    ...
    },
    {
      ...
      description: 'Increase cooling circulation flow to 500 L/min to stabilize exothermic reactor thermal escalation.',
  ...
  steps: [
    {
      ...
      description: 'Primary isolation pathway failed. Automated causal shadow-run switched mitigation vector to Bypass Line B.',
    ...
    },
    {
      ...
      description: 'Rotate bypass valve V-108 to 100% open position to direct pressure surge directly to Flare Stack FS-01.',
      assignedRole: 'Field Technician',
      isLotoRequired: true,
      lotoStatus: 'verified',
      isCompleted: false,
    ...
    },
    {
      ...
      description: 'Confirm automatic pilot ignition on Flare Stack FS-01 to safely combust vented hydrocarbon effluent.',
      assignedRole: 'Control Room Operator',
      isLotoRequired: false,
      lotoStatus: 'pending',
      isCompleted: false,
  ...
  searchCopilot: async (query: string) => {
    try {
      ...
      return {
        ...
        confidence: 0.97,
        citations: [
          { document: 'OEM-R101-REV3.pdf', section: 'Section 4.2: Overpressure Thresholds', page: 18 },
          { document: 'OISD-STD-117-SAFETY.pdf', section: 'Clause 7.3: Flare Redirection Protocols', page: 34 }
        ]
      };
    }
  }
