...
export const Navbar: React.FC = () => {
  ...
  const [notifications, setNotifications] = useState<
    Array<{ id: string; title: string; time: string; desc: string }>
  >([]);
  const initials = (currentUser?.username || 'AP').slice(0, 2).toUpperCase();
      ...
      .then(incidents => {
        const latest = [...incidents]
          ...
          .map(incident => ({
            ...
            desc: incident.outcome,
          }));

        if (currentSimulation?.simulation_id) {
          latest.unshift({
            id: currentSimulation.simulation_id,
            title: `Simulation ${currentSimulation.request.failed_asset}`,
            time: 'Active',
            desc: `${currentSimulation.scenarios.length} scenarios loaded`,
          });
        }

        setNotifications(latest.slice(0, 3));
      })
      .catch(() => {
        if (currentSimulation?.simulation_id) {
          setNotifications([
  ...
  return (
    <header className="sticky top-0 z-20 flex items-center justify-between px-6 py-3 bg-[var(--bg-surface)] border-b border-[var(--border-muted)] h-16 select-none shadow-sm">
      ...
      <div className="flex items-center gap-2">
        ...
        {/* Connection Status */}
        <div
          className={`hidden xl:flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-mono font-medium border ${
        ...
        >
          <CheckCircle2
            className={`w-3 h-3 ${connectionState === 'connected' ? 'text-emerald-400' : 'text-accent-red'}`}
          />
          <span>{connectionState === 'connected' ? 'Connected' : 'Offline'}</span>
        </div>
              ...
              >
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                  {notifications.length ? (
                    ...
                    <div className="p-3 rounded-xl bg-[#151D2A] border border-slate-800 text-[11px] text-slate-400">
                      No live incident notifications are available yet.
                    </div>
                  )}
                </div>
              ...
              >
                ...
                <div className="space-y-2 text-xs font-mono text-slate-300 mb-4">
                  ...
                  <div className="flex justify-between p-2 rounded bg-[#151D2A] border border-slate-800">
                    ...
                    <strong className="text-emerald-400">{currentUser?.role || 'Unknown'}</strong>
                  </div>
                  <div className="flex justify-between p-2 rounded bg-[#151D2A] border border-slate-800">
                    <span className="text-slate-400">Session:</span>
                    <strong className="text-blue-400">Authenticated</strong>
                  </div>
                </div>
