...
export const StatCard: React.FC<StatCardProps> = ({
  ...
  const sparklineColor = changeType === 'positive' ? '#10b981' : changeType === 'negative' ? '#ef4444' : '#3b82f6';
  return (
    <div className="enterprise-card p-5 hover:border-[var(--border-strong)] transition-colors relative overflow-hidden group">
      <div className="flex items-center justify-between mb-4 relative z-10">
        <div className="flex items-center gap-2">
          ...
          {isLive && <span className="w-1.5 h-1.5 rounded-full bg-accent-emerald animate-pulse" title="Live Data" />}
        </div>
      ...
      <div className="flex items-baseline gap-3">
        ...
        {change && (
          <span className={`text-xs font-medium px-2 py-0.5 rounded ${
            changeType === 'positive' ? 'bg-accent-emerald/10 text-accent-emerald border border-accent-emerald/20' :
            changeType === 'negative' ? 'bg-accent-red/10 text-accent-red border border-accent-red/20' :
      ...
      {description && <p className="mt-3 text-[11px] text-[var(--text-muted)] font-medium tracking-wide relative z-10">{description}</p>}
    </div>
  );
};
